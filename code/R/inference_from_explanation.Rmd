...
```{r}
df.normality %>% 
  count(structure, norm, selection) %>% 
...
```{r}
regression_table(fit_normality_selection) %>% 
  print_table()
```
...
```{r}
regression_table(fit_normality_judgment) %>% 
  print_table()
```
...
# Reference clip Learning Trial and Judgment: Dark Motion Block is top, Ball A 
...
```{r}
regression_table(fit_structure_selection) %>% 
  print_table()
```
...
```{r}
regression_table(fit_structure_judgment) %>% 
  print_table()
```
